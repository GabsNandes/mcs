...
df = pd.read_csv('/run/media/metatron/Yggdrasil/python/arboseer/auto.csv', delimiter=";", decimal=",")
df = df[(df['VL_LONGITUDE'] >= loni) & (df['VL_LONGITUDE'] <= lonf) &
                 (df['VL_LATITUDE'] >= lati) & (df['VL_LATITUDE'] <= latf)]
df.to_csv('/run/media/metatron/Yggdrasil/python/arboseer/auto.csv')
df = pd.read_csv('/run/media/metatron/Yggdrasil/python/arboseer/auto.csv')
